...
@section('content')
    <div class="max-w-4xl mx-auto">
        ...
        <div class="bg-white rounded-[2rem] shadow-sm border border-gray-100 overflow-hidden">
            <form action="{{ route('admin.blog.store') }}" method="POST" enctype="multipart/form-data" class="p-10 space-y-8">
                @csrf
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                    <div>
                        ...
                        <input type="text" name="title" value="{{ old('title') }}" required class="w-full px-6 py-4 bg-gray-50 border-0 rounded-2xl focus:ring-2 focus:ring-primary-600 outline-none" placeholder="e.g. Top 10 European Destinations for 2026">
                    </div>
                    <div>
                        <label class="block text-xs font-black uppercase tracking-widest text-gray-400 mb-2">Slug</label>
                        <input type="text" name="slug" value="{{ old('slug') }}" required class="w-full px-6 py-4 bg-gray-50 border-0 rounded-2xl focus:ring-2 focus:ring-primary-600 outline-none" placeholder="auto-generated-from-title">
                    </div>
                </div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                    ...
                    <div>
                        ...
                        <input type="text" name="author_name" value="{{ old('author_name') }}" class="w-full px-6 py-4 bg-gray-50 border-0 rounded-2xl focus:ring-2 focus:ring-primary-600 outline-none" placeholder="e.g. John Doe">
                    </div>
                </div>
                        ...
                        >
                            <template x-if="!imagePreview">
                                <div>
                                    ...
                                    <p class="text-sm font-bold text-gray-500">Drop image here or click to browse</p>
                ...
                <div>
                    ...
                    <textarea name="excerpt" rows="3" class="w-full px-6 py-4 bg-gray-50 border-0 rounded-2xl focus:ring-2 focus:ring-primary-600 outline-none" placeholder="A short summary of the blog post...">{{ old('excerpt') }}</textarea>
                </div>

                <div>
                    <label class="block text-xs font-black uppercase tracking-widest text-gray-400 mb-2">Content</label>
                    <input type="hidden" name="content" id="blog-content-input" value="{{ old('content') }}">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                        <div>
                            ...
                            <label class="flex items-center space-x-3 cursor-pointer">
                                ...
                                <input type="checkbox" name="is_published" value="1" {{ old('is_published') ? 'checked' : '' }} class="w-5 h-5 rounded-lg text-primary-600 focus:ring-primary-600">
                        ...
                        <div>
                            ...
                            <input type="datetime-local" name="published_at" value="{{ old('published_at') }}" class="w-full px-6 py-4 bg-gray-50 border-0 rounded-2xl focus:ring-2 focus:ring-primary-600 outline-none">
                        </div>
                    </div>
                </div>
...
function tagSelector() {
    return {
        selectedTags: @json(array_map('intval', old('tags', []))),
...
quill.on('text-change', function() {
    ...
    // Quill outputs <p><br></p> when empty — treat as empty
