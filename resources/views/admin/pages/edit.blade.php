        ...
        <div class="bg-white rounded-[2rem] shadow-sm border border-gray-100 overflow-hidden">
            <form action="{{ route('admin.page.update', $page->id) }}" method="POST" class="p-10 space-y-8">
                ...
                <div>
                    ...
                    <textarea name="city_specific_content" rows="5" class="w-full px-6 py-4 bg-gray-50 border-0 rounded-2xl focus:ring-2 focus:ring-primary-600 outline-none" placeholder="Unique content about why travel agents in this city choose GYF...">{{ old('city_specific_content', $page->city_specific_content) }}</textarea>
                </div>

                <div>
                    <label class="block text-xs font-black uppercase tracking-widest text-gray-400 mb-2">Additional SEO Content</label>
                    <textarea name="seo_content" rows="5" class="w-full px-6 py-4 bg-gray-50 border-0 rounded-2xl focus:ring-2 focus:ring-primary-600 outline-none" placeholder="Extra content for SEO depth...">{{ old('seo_content', $page->seo_content) }}</textarea>
                </div>
